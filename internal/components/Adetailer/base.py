...
from .utils import *
from internal.components.yolo import *
...
class AdPipelineBase():
    ...
    @property
    @abstractmethod
    def txt2img_class(self) -> Callable:
        raise NotImplementedError
    ...
    def __call__(  # noqa: C901
        ...
        if images is None:
            ...
            # self.txt2img_class.unload_lora_weights()
            ...
            for j, detector in enumerate(detectors):
                ...
                for k, mask in enumerate(masks):
                    ...
                    if inpaint_image is not None:
                        inpaint_images.append(inpaint_image)
        ...
        return ADOutput(images=final_images, init_images=init_images, inpaint_images=inpaint_images)
    ...
    def process_txt2img(
        ...
        return self.txt2img_class(**txt2img_args)
    ...
    def process_inpainting(
        ...
        inpaint_args["width"]=512
        inpaint_args["height"]=768

        if "control_image" in inpaint_args:
            if len(inpaint_args["control_image"]) > 1:
                inpaint_args["control_image"] = [
                    control_image.resize(crop_image.size)
                    for control_image in inpaint_args["control_image"]
                ]
            else:
