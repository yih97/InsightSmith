...
def train_model(
    ...
    if print_only_bool:
        ...
        log.info(run_cmd)
        # Run the command
        process = executor.execute_command(run_cmd=run_cmd)
        if process and process.poll() is None:
            gr.Warning('Training in process')
...
def lora_tab(
    ...
    with gr.Tab("Training") as training:
        gr.Markdown(
        ...
        def get_training_status():
            if executor.process is None:
                gr.Info('No training found')
                #return 'No process found'
            elif executor.process and executor.process.poll() is None:
                gr.Info('Training in progress')
                #return 'Training in progress'
            else:
                #return 'Training Completed'
                gr.Info('Training Completed')
        #setup for training status
        button_status = gr.Button("Check Training Status", variant="primary")
        # text_status = gr.Textbox(value="Training Status:", interactive=False)
        button_status.click(
            get_training_status,
            inputs=[],
            outputs=[]
